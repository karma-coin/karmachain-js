...
test("Events stream", async (t) => {
  // Subscribe to events by specific `AccountId`
  const unsubscribe = await subscribeAccountEvents(
    t.context.api,
    t.context.users[0].pair.address,
    (extrinsic, events) => {
      // This callback will be called when tx or events happend to related `AccountId`

      // Check new user event
      const newUserEvent = events.find((event) =>
        t.context.api.events.identity.newUser.is(event.event)
      );
      if (newUserEvent) {
        t.assert(newUserEvent.event.data.account_id, t.context.users[0].pair.address);
        t.assert(newUserEvent.event.data.username, t.context.users[0].username);
        t.assert(newUserEvent.event.data.phone_number, t.context.users[0].phoneNumber);
      }
